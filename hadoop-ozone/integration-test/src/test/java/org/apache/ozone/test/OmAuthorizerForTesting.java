 ...
 */
public class OmAuthorizerForTesting implements OzoneManagerAuthorizer {
  ...
  public boolean checkAccess(IOzoneObj obj, RequestContext context) {
    ...
    System.out.printf("ACCESS %s: allow %s to %s? %s [%s]%n",
        id, obj, context.getAclRights(), allowed, Thread.currentThread());
    return allowed;
  }

  @Override
  public OzoneManagerAuthorizer configure(OzoneManager om, KeyManager km, PrefixManager pm) {
    id = om.getOMNodeId();
