...
public class TestTlsConfWithNetty {
  ...
  public void testSsl() throws Exception {
    final TlsConf serverTlsConfig = SecurityTestUtils.newServerGrpcTlsConfig(true);
    final TlsConf clientTlsConfig = SecurityTestUtils.newClientGrpcTlsConfig(true);
  ...
  static void runTest(int port, TlsConf serverSslConf, TlsConf clientSslConf) throws Exception {
    final SslContext serverSslContext = serverSslConf == null? null
        : newServerSslContextBuilder(serverSslConf).build();
    final SslContext clientSslContext = clientSslConf == null? null
        : newClientSslContextBuilder(clientSslConf).build();
  ...
  static SslContextBuilder newServerSslContextBuilder(TlsConf conf) {
    final SslContextBuilder b = SslContextBuilder.forServer(
        conf.getKeyManager().getKeyCertificates().getFile(),
        conf.getKeyManager().getPrivateKey().getFile()
    );
    return b;
  }

  static SslContextBuilder newClientSslContextBuilder(TlsConf conf) {
    final SslContextBuilder b = GrpcSslContexts.forClient();
    b.trustManager(conf.getTrustManager().getTrustCertificates().getFile());
    return b;
  }
