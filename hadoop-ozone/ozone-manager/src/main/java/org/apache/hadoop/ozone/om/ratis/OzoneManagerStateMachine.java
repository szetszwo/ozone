...
public class OzoneManagerStateMachine extends BaseStateMachine {
  ...
  public synchronized void notifyTermIndexUpdated(long currentTerm, long index) {
    lastNotifiedTermIndex = assertUpdateIncreasingly(
        "lastNotified", lastNotifiedTermIndex, TermIndex.valueOf(currentTerm, index));
  }
  ...
  @Override
  protected synchronized boolean updateLastAppliedTermIndex(TermIndex termIndex) {
    return super.updateLastAppliedTermIndex(assertUpdateIncreasingly(
        "lastApplied", getLastAppliedTermIndex(), termIndex));
  }
