...
/**
 * Verifies that a connection-class heartbeat failure triggers a DNS re-resolution of the SCM peer
 * (HDDS-15533), gated by the resolve-needed flag, the passive (Recon) check, the missed-heartbeat
 * threshold, and the connection-class classifier. Because #10612 keys SCM identity on the stable
 * host:port, the trigger only asks the connection manager to rebuild the endpoint -- there is no
 * re-keying or StateContext queue migration to assert here.
 */
public class TestHeartbeatEndpointTaskDnsRefresh {
  ...
  /**
   * Drives a single heartbeat that fails with {@code failure}, with the endpoint reporting
